...
sys.path.insert(
    0,
    os.path.abspath(
        os.path.join(os.path.join(os.path.dirname(__file__), ".."), "bpepi")
    ),
)
...
parser.add_argument(
    ...
    default="200",
    dest="max_it",
    help="max number of iterations in BP update",
)
...
parser.add_argument(
    "--lam", type=float, default="1", dest="lam", help="transmission probability"
)
parser.add_argument(
    "--delta",
    type=float,
    default="0.01",
    dest="delta",
    help="probability of being source",
)
parser.add_argument(
    "--rho", type=float, default="0.1", dest="rho", help="fraction of observations"
)
...
def test(type="t"):
    ...
    if type == "t":
        num_dir = 0
        count_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "test_data/tree")
        )
        ...
        for i in range(0, num_dir):
            ...
            if (
                np.allclose(
                    np.sum(np.abs(code_marginals - sib_marginals), axis=1),
                    np.zeros(params[0]),
                    atol=1e-6,
                )
                == True
    ...
    if type == "i":
        ...
        num_dir = 0
        count_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "test_data/initialization")
        )
        for root, dirs, files in os.walk(count_path):
            for name in dirs:
                num_dir += 1
        for i in range(0, num_dir):
            ...
            code_f_inf = FactorGraph(
                params[0], params[1], contacts, list_obs_all, params[2]
            )
            ...
            for tol in tolerances:
                if np.allclose(
                    np.sum(np.abs(code_marginals_rnd - sib_marginals_rnd), axis=1),
                    np.zeros(params[0]),
                    atol=tol,
                ):
                    distance_1 = "<" + str(tol)
            ...
            for tol in tolerances:
                if np.allclose(
                    np.sum(np.abs(code_marginals_inf - sib_marginals_rnd), axis=1),
                    np.zeros(params[0]),
                    atol=tol,
                ):
                    distance_2 = "<" + str(tol)
            ...
            for tol in tolerances:
                if np.allclose(
                    np.sum(np.abs(sib_marginals_rnd - sib_marginals_inf), axis=1),
                    np.zeros(params[0]),
                    atol=tol,
                ):
                    distance_3 = "<" + str(tol)
    ...
    if type == "s":
        ...
        status_nodes = generate_one_conf(
            g, lamb=lam, T=T + 1, percentage_infected=delta
        )
        list_obs = generate_obs(status_nodes, frac_obs=rho)
        list_obs_all = generate_obs(status_nodes, frac_obs=1.0)
